...
static const std::regex SATO_NAMES("^SATO$|^SATO$|^SATOCOIN$|^#SATO$|^#SATO$|^#SATOCOIN$");
...
bool IsRootNameValid(const std::string& name)
{
    return std::regex_match(name, ROOT_NAME_CHARACTERS)
        ...
        && !std::regex_match(name, SATO_NAMES);
}
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SATO_R); // r
    vchMessage.push_back(SATO_V); // v
    vchMessage.push_back(SATO_N); // n
    vchMessage.push_back(SATO_Q); // q

    vchMessage.insert(vchMessage.end(), ssAsset.begin(), ssAsset.end());
    script << OP_SATO_ASSET << ToByteVector(vchMessage) << OP_DROP;
}
...
bool TransferAssetFromScript(const CScript& scriptPubKey, CAssetTransfer& assetTransfer, std::string& strAddress)
{
    ...
    if (AreTransferScriptsSizeDeployed()) {
        ...
        // This fixes that issue (https://github.com/SatoProject/Sato/issues/752)
...
{
    // Must contain at least 3 outpoints (SATO burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
bool CTransaction::VerifyNewAsset(std::string& strError) const {
    // Issuing an Asset must contain at least 3 CTxOut( Sato Burn Tx, Any Number of other Outputs ..., Owner Asset Tx, New Asset Tx)
    if (vout.size() < 3) {
...
{
    // Issuing an Asset must contain at least 2 CTxOut( Sato Burn Tx, New Asset Tx, Any Number of other Outputs...)
    if (vout.size() < 2) {
...
bool CTransaction::VerifyNewRestrictedAsset(std::string& strError) const {
    // Issuing a restricted asset must cointain at least 4 CTxOut(Sato Burn Tx, Asset Creation, Root Owner Token Transfer, and CNullAssetTxVerifierString)
    if (vout.size() < 4) {
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( Sato Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct SATO Amount
...
{
    // Verify 'splq' is in the transaction
...
{
    // Verify 'splr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
bool CreateAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewAsset> assets, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sato address: ") + change_address);
            return false;
        }
    } else {
    ...
    // Check to make sure the wallet has the SATO required by the burnAmount
...
bool CreateReissueAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueAsset& reissueAsset, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Sato address: ") + address);
        return false;
    }
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::vector<std::pair<CNullAssetTxData, std::string> >* nullAssetTxData, std::vector<CNullAssetTxData>* nullGlobalRestrictionData)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any SATO, transfering an asset requires a network fee"));
        return false;
    }
    ...
    // If assetTxData is not nullptr, the user wants to add some OP_SATO_ASSET data transactions into the transaction
    if (nullAssetTxData) {
        std::string strError = "";
        int nAddTagCount = 0;
    ...
    // nullGlobalRestiotionData, the user wants to add OP_SATO_ASSET OP_SATO_ASSET OP_SATO_ASSETS data transaction to the transaction
...
void CNullAssetTxData::ConstructGlobalRestrictionTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_SATO_ASSET << OP_RESERVED << OP_RESERVED << ToByteVector(vchMessage);
}
...
void CNullAssetTxVerifierString::ConstructTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_SATO_ASSET << OP_RESERVED << ToByteVector(vchMessage);
}
